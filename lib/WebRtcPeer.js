...
function WebRtcPeer(mode, options, callback) {
  ...
  this.generateOffer = function (callback) {
    ...
    //safari WebRtcPeerRecvonly fix
    if (mode === 'recvonly' && pc.addTransceiver &&
      browserDependantConstraints.offerToReceiveAudio) {
      pc.addTransceiver('audio')
    }
    if (mode === 'recvonly' && pc.addTransceiver &&
      browserDependantConstraints.offerToReceiveVideo) {
      ...
      if (remoteVideo.srcObject != event.streams[0]) {
        ...
        remoteVideo.onloadedmetadata = function () {
          ...
          remoteVideo.play()
            ...
            .catch(function (err) { 
              logger.error("Error while trying to play remote video:", err) 
            })
        }
      }
    }
  ...
  this.showRemoteVideo = function () {
    remoteVideo.srcObject = this.getRemoteStream()
    remoteVideo.muted = false

    if (typeof AdapterJS !== 'undefined' && AdapterJS
      .webrtcDetectedBrowser === 'IE' && AdapterJS.webrtcDetectedVersion >= 9
    ) {
      remoteVideo = attachMediaStream(remoteVideo, this.getRemoteStream());
    }
  };
